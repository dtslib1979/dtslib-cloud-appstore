...
### 9.7 아이디어 진화 기록 — "Appstore에서 Studio로"

> 이 섹션은 2026-03-03 브레인스토밍 세션의 논점과 결론을 기록한다.
> 헌법 제1조 "삽질/방향 전환 전부 남긴다"에 의거.
...
Day 0 (2025-12-03)
  "웹에서 돌아가는 도구 모음 만들자"
  → 레포명 dtslib-cloud-appstore 확정
  → 타입 분류: apk-lab(온디바이스) / cloud-appstore(웹 기반)
  → 이 시점에서 "appstore"는 합리적 네이밍

Day 1~60
  9개 도구 개발, PWA 2-Track, 대시보드, Pipeline
  → 카드형 UI, 카테고리 필터, 버전 뱃지
  → "스토어" 문법으로 자연스럽게 구축

Day 60~90
  사용하면서 의문 발생:
  → "PWA가 꼭 필요한가? SW 없는 도구가 대부분인데"
  → "브라우저에서만 할 수 있는 게 뭔데?"
  → "이건 앱을 파는 게 아니라 제작 환경 아닌가?"

Day 90 (2026-03-03) — 전환점
  → "프로그래밍이 아니라 방송 소재를 만든다"
  → Browser = Studio 프레이밍 확립
```
...
| 시점 | 진단 | 결론 |
|------|------|------|
| ChatGPT | "스토어를 버려라. Studio로 재정의해라" | 이름 자체를 바꾸라 |
| Claude | "이름은 유지. 프레이밍만 전환" | 레포명 = 기술 식별자, 정체성 = 백서에서 정의 |

**최종 결론:** 레포명 `dtslib-cloud-appstore`는 유지한다.

이유:
1. Twin Store 타입 분류(apk-lab=온디바이스, cloud-appstore=웹)는 원래부터 맞았다
2. GitHub rename = URL/참조/북마크 전부 파손 → 비용 대비 실익 없음
3. 408개 커밋의 서사가 이 이름으로 기록됨 (헌법 제1조)
4. 정체성은 코드와 백서에서 정의하지, 레포명에서 정의하지 않는다

#### 논쟁 2: "도구를 통합 파이프라인으로 묶어야 하나"

| 시점 | 주장 | 반론 |
|------|------|------|
| ChatGPT | "9개 도구를 하나의 방송 프로그램으로 재배열" | 모놀리스 = 유지보수 지옥, "인샷보다 덜 귀찮은가?" 실패 |
| ChatGPT | "단일 파이프라인 + 자동 흐름 + 실시간 피드백" | AI 요약 = 월 비용 발생, 구독 의존 → $0 원칙 위반 |
| Claude | "도구 사이 연결이 필요한 지점은 제한적" | lecture-shorts → clip-shorts 핸드오프 정도 |

**최종 결론:** 독립 도구 구조 유지. 통합 파이프라인 불필요.

이유:
1. 각 도구가 독립적 = 유지보수 단순, 리스크 분산
2. 1인 운영에 모놀리스 파이프라인은 과잉 설계
3. "기능은 빼는 것이 가치다" — 기능을 합치는 건 정반대

#### 논쟁 3: "정체성이 불명확하다"

| 시점 | 주장 | 반론 |
|------|------|------|
| ChatGPT | "UI가 스토어 문법(카드, 필터, 뱃지)인데 내부 실험실" | UI 문법 ≠ 정체성. 카드 그리드는 네비게이션 패턴일 뿐 |
| ChatGPT | "실험실이면 최근 사용, 작업 세션이 전면에 와야" | 과잉 설계. 도구 9개에 세션 관리는 불필요 |
| Claude | "CLAUDE.md에 이미 정의됨: 개인 제작 내부 콘솔" | 문서 + 코드 + 백서에서 일관 |

**최종 결론:** 정체성은 명확하다. 표면(UI)과 내면(목적)의 괴리는 존재하나, 9개 도구 규모에서 UI 리디자인은 ROI 없음.

#### 논쟁 4: "브라우저의 실시간 인터랙션을 안 쓰고 있다"

| 주장 | 사실 |
|------|------|
| "터미널 배치의 GUI 포장일 뿐" | **틀림.** FFmpeg 도구 = 드래그앤드롭, 실시간 프리뷰, 프로그레스 UI |
| "실시간 인터랙션 미활용" | **부분적 사실.** 카메라/마이크/PiP 등 아직 미사용 엔진 있음 |

**최종 결론:** 기존 도구는 이미 브라우저 인터랙션을 쓰고 있다. 미사용 엔진(§9.3 카탈로그)은 향후 도구에서 활용.

#### 합의점 (ChatGPT + Claude 공통)

양쪽이 동의한 것:

1. **브라우저 = 무료 런타임 엔진** — Chrome이 GPU/오디오/카메라/센서를 공짜로 제공
2. **Termux(배치) vs 브라우저(실시간)** — 역할 분담이 명확
3. **"앱스토어"보다 "제작 환경"이 실체에 가깝다** — 소비자용이 아닌 제작자용
4. **기술 스택은 바꿀 필요 없다** — HTML/JS/CSS, 빌드 도구 없음, CDN only
...
> **레포명은 출생 신고서다. 그 뒤로 어떤 사람이 됐는지는 서사가 증명한다.**
